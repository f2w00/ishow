...
} from "../../platform/base/project/project";
...
import {
    LocalEvents,
    renderEvents,
} from "../../platform/ipc/events/ipc.events";
...
export class WorkspaceManager implements IWorkspaceManager {
    ...
    initBind() {
        ...
        ipcClient.handleRender(
            renderEvents.benchEvents.clientInfo,
            (_, ...args) => {
                return {
    ...
    toStart() {
        if (
            this.onStart &&
            this.onStart.length > 0 &&
            !this.loadedProjects.has(this.onStart)
        ) {
            ipcClient.emitLocal(
    ...
    createProject(projectName: string, projectType: string) {
        let fileName =
            this.workspace.storagePath + `\\${projectName}` + `\\.${projectType}`;
    ...
    loadProject(fileName: string) {
        ...
        sharedData.set(
            ...
            project
                ...
                : project2
                    ? project2
                    : this.workspace.storagePath + "/your_project"
        );
        return {
...
export class GlobalWorkspaceManager {
    ...
    initBind() {
        ipcClient.handleRender(
            renderEvents.workspaceEvents.openFolder,
            (_, fileName: string) => {
        ...
        if (workspace) {
            ...
            if (recent) {
                recent.forEach((value) => {
                    GlobalWorkspaceManager.recent.set(
    ...
    static updateStore() {
        ClientStore.set(storeNames.moduleStoreName, "recentManagers", [
